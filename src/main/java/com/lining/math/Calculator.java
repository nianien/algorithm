...
import com.nianien.core.container.IDContainer;
...
public class Calculator {
	 ...
	 * 存储变量的容器
	 */
	private static IDContainer<Double> idc = new IDContainer<Double>("@", "");
	/**
	 * 无符号实数的正则表达式
	 */
	 ...
	 * @return
	 */
	private static String initVariable(String expression) {
	...
	private static String doCalculate(String expression) {
		// 按优先级运算, 从左到右进行运算
		expression = doCalcalate(expression, new char[] { '^' }, true);// 幂运算
		expression = doCalcalate(expression, new char[] { '*', '/' }, false);// 乘除运算
		expression = doCalcalate(expression, new char[] { '+', '-' }, false);// 加减运算
		return expression;
	}
	 ...
	 * @return
	 */
	private static String doCalcalate(String expression, char[] ops,
			boolean rightToLeft) {
		int index = rightToLeft ? StringUtils.lastIndexOfAny(expression, ops)
				: StringUtils.indexOfAny(expression, ops);
		if (index == -1)
			return expression;
		// 运算符
		...
		return doCalcalate(left + atomCalculate(p1, p2, op) + right, ops,
				rightToLeft);
	}
	 ...
	 * @return
	 */
	private static String atomCalculate(String p1, String p2, char op) {
		double x = idc.removeValue(p1);
		double y = idc.removeValue(p2);
		switch (op) {
		case '+':
			return idc.putValue(x + y);
		case '-':
			return idc.putValue(x - y);
		case '*':
			return idc.putValue(x * y);
		case '/':
			return idc.putValue(x / y);
		case '^':
			return idc.putValue(Math.pow(x, y));
		default:
			throw  ExceptionHandler.throwException("Unknown operator: " + op);
		}
	}
	 ...
	 * @return
	 */
	private static boolean checkNumber(String expression) {
		// 表达式中的数字字符串
	 ...
	 * @return
	 */
	private static EnumSet<Type> nextType(Type type, int unmatched) {
		EnumSet<Type> expected = EnumSet.noneOf(Type.class);
		switch (type) {
	...
	private static enum Type {
		...
		static Type typeOf(char ch) {
			...
			case ')':
				return Type.RP;
			default:
				throw  ExceptionHandler.throwException("Unknown type:" + ch);
			}
		}
	}
}
